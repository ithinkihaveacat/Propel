...
describe('Test Successful Permission State', () => {
  ...
  it('subscribe() should throw an error due to no manifest', function() {
    ...
    .then(subscription => {
      throw new Error('Subscription passed successfully with no manifest defined');
    })
    .catch(err => {
      err.message.should.equal('Registration failed - gcm_sender_id not found in manifest');
    });
  });
  ...
  it('statuschange event should fire after the subscription fails', function(done) {
    const pushClient = new window.goog.propel.Client();
